...
libmemory.so: memory.c
	gcc -g -Werror -shared -O3 -fPIC -o libmemory.so -fsanitize=address -static-libasan memory.c

random: randomalloc.c libmemory.so
	gcc -g -O3 -L`pwd` -Wl,-rpath=`pwd` -o random randomalloc.c -lmemory -fsanitize=address -static-libasan     
	gcc -g -O3 -L`pwd` -Wl,-rpath=`pwd` -o small smallobject.c -lmemory -fsanitize=address -static-libasan

run: random
	/usr/bin/time -v ./random
#	/usr/bin/time -v ./small
